// ============================================================================
// admin-exercices.js
// Gestion des exercices (bibliothèque PDF) — page admin-exercices.html
...
async function init() {
  ...
  bindOverlayClose("exercise-modal");
  document.getElementById("btn-add-exercise").addEventListener("click", () => openForm());
  document.getElementById("btn-close-modal").addEventListener("click", () => closeModal("exercise-modal"));
  document.getElementById("btn-cancel").addEventListener("click", () => closeModal("exercise-modal"));
  document.getElementById("exercise-form").addEventListener("submit", onSubmit);
  document.getElementById("searchExercise").addEventListener("input", renderTable);
  document.getElementById("f-pdf-file").addEventListener("change", onPdfFileChange);
  document.getElementById("f-correction-file").addEventListener("change", onCorrectionFileChange);
...
async function loadExercises() {
  ...
  try {
    exercises = await getAllExercicesLib();
  } catch (err) {
    console.error(err);
    exercises = [];
    toastError("Impossible de charger les exercices.");
  }
  renderTable();
}

function renderTable() {
  const term = document.getElementById("searchExercise").value.trim().toLowerCase();
  const filtered = term
    ? exercises.filter(e =>
  ...
  if (!filtered.length) {
    tbody.innerHTML = `<tr><td colspan="7"><div class="empty-state"><div class="emoji">✏️</div>Aucun exercice trouvé.</div></td></tr>`;
    return;
  }
  ...
  tbody.innerHTML = filtered.map(e => `
    <tr>
      ...
      <td class="row-actions">
        ...
        ${e.correction ? `<a class="btn btn-secondary btn-sm" href="${e.correction}" target="_blank" rel="noopener">Corrigé</a>` : ""}
  ...
  tbody.querySelectorAll("[data-edit]").forEach(btn =>
    btn.addEventListener("click", () => openForm(exercises.find(e => e.id === btn.dataset.edit))));

  tbody.querySelectorAll("[data-delete]").forEach(btn =>
    btn.addEventListener("click", async () => {
      const ex = exercises.find(e => e.id === btn.dataset.delete);
      const ok = await confirmDelete(`Supprimer l'exercice « ${ex?.titre || ""} » ?`);
      if (!ok) return;
      try {
        await deleteExerciceLib(ex.id, ex.titre);
        toastSuccess("Exercice supprimé.");
        await loadExercises();
      } catch (err) {
        console.error(err);
...
function openForm(ex = null) {
  ...
  document.getElementById("modal-title").textContent = ex ? "Modifier l'exercice" : "Ajouter un exercice";
  ...
  document.getElementById("f-correction").value = ex?.correction || "";
  document.getElementById("f-correction-file").value = "";
  document.getElementById("correctionUploadStatus").textContent = ex?.correction ? "Corrigé actuel conservé si aucun nouveau fichier n'est choisi." : "";
...
/* ---- Sélection d'un nouveau corrigé : simple indication du nom de fichier ---- */
function onCorrectionFileChange(e) {
  const file = e.target.files?.[0];
  const status = document.getElementById("correctionUploadStatus");
  if (!file) return;
  status.textContent = `Fichier sélectionné : ${file.name}`;
}
...
async function onSubmit(e) {
  ...
  const correctionStatus = document.getElementById("correctionUploadStatus");
  const pdfFile = document.getElementById("f-pdf-file").files[0];
  const correctionFile = document.getElementById("f-correction-file").files[0];

  submitBtn.disabled = true;
  let pdfUrl = document.getElementById("f-pdf").value.trim();
  let correctionUrl = document.getElementById("f-correction").value.trim();
  ...
  try {
    ...
    if (correctionFile) {
      correctionStatus.textContent = "Envoi du corrigé en cours…";
      correctionUrl = await uploadPdfToCloudinary(correctionFile);
    ...
    if (editingId) {
      await updateExerciceLib(editingId, data);
      toastSuccess("Exercice mis à jour.");
    } else {
      await addExerciceLib(data);
      toastSuccess("Exercice ajouté.");
    }
